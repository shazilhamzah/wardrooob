...
def measurements_to_betas(target_measurements):
    """
    Helper function to load the SMPL model and optimize betas for target measurements.
    Used by preset_lookup_test.py.
    """
    base_dir = os.path.dirname(os.path.abspath(__file__))
    model_dir = os.path.abspath(os.path.join(base_dir, "../models"))
    smpl_model, num_betas = load_smpl_model(model_dir)
    return optimize_betas_for_measurements(smpl_model, target_measurements, num_betas)
