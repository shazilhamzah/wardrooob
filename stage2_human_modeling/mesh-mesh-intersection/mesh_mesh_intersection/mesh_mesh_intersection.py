...
class MeshMeshIntersectionFunction(autograd.Function):
    ...
    def forward(ctx, query_triangles, target_triangles, print_timings=False,
                ...
                *args, **kwargs):
        if not HAS_CUDA_EXT:
            raise RuntimeError("mesh_mesh_intersect_cuda is not installed/available. You cannot run mesh-to-mesh intersection on GPU without compiling the CUDA extension.")
